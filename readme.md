# Assessing missing data for alternative music groups

## Resources
- [Knowledge graph refinement: A survey of approaches and evaluation methods](https://content.iospress.com/articles/semantic-web/sw218)
- [Knowledge graph expansion and enrichment](https://project.inria.fr/bda2017/files/2017/11/tutorial2.pdf)
- [Microsoft: Building a Large-scale, Accurate and Fresh Knowledge Graph](https://kdd2018tutorialt39.azurewebsites.net/KDD%20Tutorial%20T39.pdf)

### Ontology
- [Linked Open Vocabularies (LOV)](https://lov.linkeddata.es)
- [Music ontology](http://musicontology.com)

### RDF file generation from the database
- [Musicbrainz database download](https://beta.musicbrainz.org/doc/MusicBrainz_Database/Download)
- [Musicbrainz RDF generation](https://wiki.musicbrainz.org/LinkedBrainz/RDF)

### Interlinking knowledge graphs
- [DBpedia interlinking](https://wiki.dbpedia.org/services-resources/interlinking)
- [Linked Data: Evolving the Web into a Global Data Space](http://linkeddatabook.com/editions/1.0/)
- [LIMES](http://aksw.org/Projects/LIMES.html)

### External sources to graphs
- [Synthesizing Knowledge Graphs from WebSources with the MINTE+ Framework](https://www.researchgate.net/profile/Diego_Collarana/publication/325381996_Synthesizing_Knowledge_Graphs_from_web_sources_with_the_MINTE_framework/links/5b290b4b0f7e9b1d003575ca/Synthesizing-Knowledge-Graphs-from-web-sources-with-the-MINTE-framework.pdf)
- [Improving knowledge base with information extraction](https://cs.nyu.edu/media/publications/li_xiang.pdf)
- [Open-World Knowledge Graph Completion](https://arxiv.org/pdf/1711.03438.pdf)
- [Discogs API](https://www.discogs.com/developers/)
- [Last.fm API](https://www.last.fm/api)
